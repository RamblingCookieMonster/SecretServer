function Set-Secret {
    ...
    process {
        ...
        if($PSCmdlet.ShouldProcess( "Processed the Secret '$($Secret | Out-String)'",
            ...
            $Prompt = "Are you REALLY sure you want to change existing`n" +
                        "'$($Secret | Out-String)`n" +
                        "with changes:`n$NewSecretPropsString'?"
            if($Force -Or $PSCmdlet.ShouldContinue($Prompt, "Processing '$($Secret | Out-String)'", [ref]$ConfirmAll, [ref]$RejectAll)) {
                if($SecretName) {
